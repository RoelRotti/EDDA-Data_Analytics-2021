...
When you want to test whether your test statistics can be obtained from a certain distribution (this distribution is the H0). Then mimic these test statistic values by computing it over pseudo observations from the H0 distribution (often rexp() or rnorm())
...
Assumptions: - Normality (check with QQplot/homoscedasticity or Shapiro-Wilk test (/ Boxplot / Histogram))
...
(generalization = Kruskall-Wallis)
...
Generalization of the two-sample t-test. The test statistic for the anova test is the F statistic = "Between groups Sum of Squares"/ "Within groups Sum of Squares". One sided, so the larger F the more evidence for rejecting H0.
...
plot(fitted(model),residuals(model), main= "Residuals vs fitted") # Plot of residuals vs fitted values: check for homoscedasticity
```
